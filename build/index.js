...
function Edit({
  ...
  const colors = color_block_colors.map(color => {
    return {
      name: color.name,
      color: '#' + color.hex
    };
  ...
  const getColor = id => {
    const match = color_block_colors.filter(color => color.id === id);
    return '#' + match[0].hex;
  };
  const setColor = hex => {
    hex = hex.replace('#', '');
    const match = color_block_colors.filter(color => color.hex === hex);
    const id = match[0].id;
    setAttributes({
      color: id
    });
...
module.exports = /*#__PURE__*/JSON.parse('{"$schema":"https://schemas.wp.org/trunk/block.json","apiVersion":3,"name":"jhu/color-block","version":"0.0.0","title":"Color Block","category":"jhu","description":"Displays a color tile.","example":{},"supports":{"html":false},"textdomain":"color-block","editorScript":"file:./index.js","editorStyle":"file:./index.css","style":"file:./style-index.css","viewScript":"file:./view.js","render":"file:./render.php","attributes":{"color":{"type":"string"}}}');
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = globalThis["webpackChunk_johnshopkins_color_block"] = globalThis["webpackChunk_johnshopkins_color_block"] || [];
